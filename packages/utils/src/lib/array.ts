...
export const getIth =
  ...
  <T>(a: T[] | readonly T[], e?: string): T => {
    ...
    if (isNone(r))
      throw new Error(
        e || e0 || `getIth expects ${i} elements, got ${a.length}`
      );
    return a[i];
  };
...
export const castNonEmptyArray = <T>(a: T[], e?: string) =>
  castNonEmptyArrayC(e)(a);
export const castNonEmptyArrayC =
  (e?: string) =>
  <T>(a: T[]): NonEmptyArray<T> => {
    if (a.length === 0)
      throw new Error(e || 'castNonEmptyArray expects non-empty array');
