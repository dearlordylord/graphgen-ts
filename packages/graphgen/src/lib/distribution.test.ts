...
const seed = rngStateFromSeed(castSeed(hash('seed1')));
...
describe('distribution', () => {
  describe('barabasiAlbert', () => {
    it('returns index of a very very heavy node', () => {
      ...
      const seed0 = rngStateFromSeed(castSeed(hash('seed1')));
      expect(ba(seed0)[0]).toBe(castNonNegativeInteger(420));
    });
  });
  describe('biasedDistribution', () => {
    it('returns 0 when K is 0 and n is any value', () => {
      const biasedDistribution = (n: Random01) =>
        defBiasedDistribution(noll)(unwrapDecimal01(n))(seed)[0];
    ...
    it('returns 1 when K is 1 and n is any value', () => {
      const biasedDistribution = (n: Random01) =>
        defBiasedDistribution(one)(unwrapDecimal01(n))(seed)[0];
    ...
    it('returns n when K is 0.5 and n is any value', () => {
      const biasedDistribution = (n: Random01) =>
        defBiasedDistribution(middle)(unwrapDecimal01(n))(seed)[0];
    ...
    it('returns a result biased towards 1 when K is close to 1', () => {
      ...
      const [b1, b2, b3] = pipe(
        ...
        RA.map(
          (k) => (seed: RngState) =>
            defBiasedDistribution(castDecimal01(k))(unwrapDecimal01(middleR))(
              seed
            )
        ),
        ST.sequenceArray,
